...
```{r exp_plot, echo = T, fig.align='center'}
...
```{r join_data, echo=T,rows.print = 5}
...
<i class='fa fa-envelope-open'> </i> ttt

```{r nested_models, echo = T, print.rows = 10}
...
models <- list(
      ...
      ca_dplbar_bfihost = function(x){lm(peak_flow_cumecs_log ~ 
                                               ca_log + DPLBAR + BFIHOST, data = x)},
      ca_dplbar_bfihost_farl = function(x){lm(peak_flow_cumecs_log ~ 
                                                    ca_log + DPLBAR + BFIHOST + FARL, data = x)},
      ca_dpsbar_bfihost_farl_fpext = function(x){lm(peak_flow_cumecs_log ~ 
                                                          ca_log + DPSBAR + BFIHOST + FARL + FPEXT, data = x)},
      ca_dpsbar_bfihost_farl_fpext_saar = function(x){lm(peak_flow_cumecs_log ~ 
                                                          ca_log + DPSBAR + BFIHOST + FARL + FPEXT + SAAR, data = x)},
      ca_dpsbar_bfihost_farl_fpext_meanMax_p = function(x){lm(peak_flow_cumecs_log ~ 
                                                          ca_log + DPSBAR + BFIHOST + FARL + FPEXT + mean_max_p_mm, data = x)},
...
```{r apply_models, echo = T, rows.print = 7}
...
```{r model_assessment, echo=T, fig.height = 8, fig.align='center'}

uk_coefficients <- uk_nest %>% select(-performance) %>% tidyr::unnest(coefficients)
uk_performance <- uk_nest %>% select(-coefficients) %>%  tidyr::unnest(performance)
...
print(unique(uk_coefficients$id_model))
